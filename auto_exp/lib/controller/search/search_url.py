...
class Search(object):
    ...
    def run_search_sub(self, domain):
        sub_search = S_Subscan(domain, self.config.path +
    ...
    def run_zoomeye_websearch(self, string, page):
    ...
    def run_zoomeye_hostsearch(self, string, page):
        zoomeye = W_Zoomeye(self.config.zoom_user, self.config.zoom_password)
        ip = []
        for i in range(int(page)):
            ip += zoomeye.host_search(string, i + 1)
        print ip
        return ip

...
class D_Mysql(object):
	def __init__(self,user,password,host,port,db):
		...
		try:
			...
			self.__output.print_error("MySQL Error " + e.args[0] + ':' + e.args[1])
	...
	def db_stop(self):
		if self.__isconnected :
			...
			self.__output.print_warning("You have not connected to MySQL Database")
	...
	def db_query(self,query):
		if self.__isconnected :
			try :
				...
				self.__output.print_error("MySQL Error " + str(e.args[0]) +":"+str(e.args[1]))
	...
	def db_insert(self,table,data):
		if self.__isconnected :
			...
			query = 'insert into `'+ table +'` ('+ ','.join(name) +') values ('+','.join(value)+')'
	...
	def db_update(self,table,data,where = '1=2'):
		if self.__isconnected :
			...
			#print ','.join(set)
			query = 'update `' +table+'` set '+','.join(set)+' where '+where
			return  self.db_execute(query)
		else :
			self.__output.print_warning("You have not connected to MySQL Database")
